...
```vue
<cbi-line-chart :data="chartData" width="100px" :events="chartEvents"></cbi-line-chart>
```
...
It should be noted that the set of identity elements will be added to each measures, such as a single-dimension multi-measures line chart. The two lines will display the corresponding identification elements. If the settings are displayed on only one indicator line, you can use `extend` and set series.
...
?> **import styles first**
<br>`import 'cbi-charts/lib/style.css'`
