...
function getTooltip (dataType, digit, dataStore, measures, color, labelMap) {
  return {
    formatter (item) {
...
export const map = (columns, rows, settings, extra) => {
  ...
  const tooltip = tooltipVisible && getTooltip(dataType, digit, dataStore, measures, color, labelMap)
  const legend = legendVisible && getLegendMap({ measures, legendName, labelMap })
  const seriesParams = {
    position,
    selectData,
    label,
