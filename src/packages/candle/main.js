...
function getCandleTooltip (args) {
  const { measures, dataType, digit, labelMap } = args
  return {
    trigger: 'axis',
    ...
    formatter (options) {
      ...
      options.forEach(option => {
        ...
        if (componentSubType === 'candlestick') {
          tpl.push('<br>')
          measures.slice(0, 4).forEach((m, i) => {
...
export const candle = (columns, rows, settings, status) => {
  ...
  const values = []
  const volumes = []
  const candlemeasures = measures.slice(0, 4)
  const volumemeasures = measures[4]

  if (isLiteData) {
    rows.forEach(row => {
      const itemResult = []
      dims.push(row[columns.indexOf(dimension)])
      candlemeasures.forEach(item => { itemResult.push(row[columns.indexOf(item)]) })
      values.push(itemResult)
      if (volumemeasures) volumes.push(row[columns.indexOf(volumemeasures)])
    })
  } else {
    rows.forEach((row, index) => {
      const itemResult = []
      dims.push(row[dimension])
      candlemeasures.forEach(item => { itemResult.push(row[item]) })
      values.push(itemResult)
      if (volumemeasures) {
        const status = row[measures[0]] > row[measures[1]] ? 1 : -1
        volumes.push([index, row[volumemeasures], status])
      }
    })
  }

  const legend = legendVisible && getCandleLegend({ showMA, MA, legendName, labelMap })
  const tooltip = tooltipVisible && getCandleTooltip({ measures, dataType, digit, labelMap })
