...
function getData (args) {
  const { rows, innerXAxisList, innerYAxisList, xDim, yDim, measures, type, extrameasures } = args
  let result = null
  if (type === 'cartesian') {
    result = rows.map(row => {
      const xIndex = innerXAxisList.indexOf(row[xDim])
      const yIndex = innerYAxisList.indexOf(row[yDim])
      const value = measures ? row[measures] : 1
      const extraData = extrameasures.map(m => row[m] || '-')
...
function getTooltip (args) {
  const { dataType, innerXAxisList, innerYAxisList, digit, extrameasures, measures } = args

  return {
    trigger: 'item',
    formatter ({ color, data: { value: [xDim, yDim, value, ...extraData] } }) {
      //9.27未支持tooltip数据居右显示
      const tpl = []
      tpl.push(`${innerXAxisList[xDim]} ~ ${innerYAxisList[yDim]}<br>`)
      extrameasures.forEach((m, index) => {
        tpl.push(`${m}: ${extraData[index]}<br>`)
      })
      tpl.push(`${itemPoint(color)} ${measures}: ${getFormated(value, dataType, digit)}<br>`)
      return tpl.join('')
    }
  }
}
...
export const heatmap = (columns, rows, settings, status) => {
  ...
  let chartData = []
  // add extrameasures prop for data which only display in tooltip
  const extrameasures = []
  const mainColumn = dimension.concat([measures])
  columns.forEach(column => {
    if (!~mainColumn.indexOf(column)) extrameasures.push(column)
  })
  ...
  let measuresList = measures ? rows.map(row => row[measures]) : [0, 5]
  if (!measuresList.length) measuresList = [0]
  const innerMin = min || Math.min.apply(null, measuresList)
  const innerMax = max || Math.max.apply(null, measuresList)
