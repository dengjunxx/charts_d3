...
function getScatterYAxis (args) {
  ...
  return {
    ...
    name: yAxisName,
    axisLabel: {
      formatter (val) { return getFormated(val, dataType[measures[0]], digit) }
    }
  }
}
...
function getScatterSeries (args) {
  ...
  const extrameasures = columns.filter(column => {
    return !~measures.indexOf(column) && column !== dimension
  })
  const numbers = []
  dataLabels.forEach(dataLabel => {
    rows[dataLabel].forEach(row => {
      numbers.push(row[measures[1]])
    })
  })
  ...
  dataLabels.forEach(dataLabel => {
    ...
    itemData.forEach(item => {
      ...
      itemResult.value.push(item[dimension], item[measures[0]], item[measures[1]])
      extrameasures.forEach(ext => { itemResult.value.push(item[ext]) })
      itemResult.symbolSize = symbolSize || (item[measures[1]] / maxNum) * symbolSizeMax
