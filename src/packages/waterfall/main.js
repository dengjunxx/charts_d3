...
function getWaterfallSeries (args) {
  ...
  let assistData
  let mainData
  const rowData = rows.map(row => row[measures])

  if (remainStatus === 'have-remain') {
    assistData = [0].concat(rows.map(row => {
      totalNumTemp -= row[measures]
      return totalNumTemp
    })).concat([0])
    mainData = [totalNum].concat(rowData).concat([totalNum - dataSum])
  } else {
    assistData = [0].concat(rows.map(row => {
      dataSumTemp -= row[measures]
      return dataSumTemp
    }))
...
export const waterfall = (columns, rows, settings, extra) => {
  ...
  measuresTemp.splice(measuresTemp.indexOf(dimension), 1)
  const measures = measuresTemp[0]
  const yAxisName = measures
  const tooltip = tooltipVisible && getWaterfallTooltip(dataType, digit)
  const dataSum = parseFloat(rows.reduce((pre, cur) => {
    return pre + Number(cur[measures])
  }, 0).toFixed(digit))
